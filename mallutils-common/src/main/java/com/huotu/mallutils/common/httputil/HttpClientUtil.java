...
package com.huotu.mallutils.common.httputil;
...
//            UrlEncodedFormEntity encodedFormEntity = new UrlEncodedFormEntity(nameValuePairs, StringUtil.UTF8);
    ...
    public HttpResult get(String url, Map requestMap) {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                if (entry.getValue() != null) {
                    if (index == 0) {
                        ...
                        finalUrl.append("&").append(entry.getKey()).append("=").append(URLEncoder.encode(String.valueOf(entry.getValue()), "utf-8"));
                    }
                }
                index++;
            }
            ...
            msg = e.getMessage();
            log.error("POST错误---" + e.getMessage(), e);
        } finally {
            try {
                if (response != null) {
                    response.close();
